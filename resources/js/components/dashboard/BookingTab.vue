...
import { PlusCircle, Plus, Trash, Sparkles } from '@lucide/vue';
...
const bookingChampionships = computed(() => {
    ...
    return props.championships.filter((c) => {
        const matchesShow =
            isPLE ||
            !c.show_id ||
            c.show_id === Number(bookingShowSelect.value);
...
const getCompetitorNameBySlot = (slot: '1' | '2' | '3' | '4') => {
    ...
    if (isTeamMatchVal && !isFactionBasedVal) {
        ...
        if (!isTeamMatchVal) {
            const s = props.superstars.find(
                (superstar) => superstar.id === Number(idVal),
            );
...
const getCompetitorImage = (match: StagedMatch | null, slot: 1 | 2 | 3 | 4) => {
    ...
    if (isTeamMatchVal && !isFactionBasedVal) {
        ...
        if (superstarIds && superstarIds.length > 0) {
            const firstSuperstar = props.superstars.find(
                (s) => s.id === superstarIds[0],
            );
        ...
        if (!isTeamMatchVal) {
            const s = props.superstars.find(
                (superstar) => superstar.id === targetId,
            );

            return s ? s.image : null;
        } else {
            const t = props.teams.find((team) => team.id === targetId);
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        >
            <div>
                ...
                >
                    <PlusCircle class="h-4 w-4 text-amber-400" /> Show Booking
                    Manager
                </h3>
                <p class="text-[10px] text-slate-400">
                    Configure parameters, stage individual match records, and
                    lock cards down to compile metrics output.
                </p>
            </div>
            ...
            >
                <div>
                    ...
                    >
                        ...
                        <option v-for="sh in shows" :key="sh.id" :value="sh.id">
                            {{ sh.name }}
                        </option>
                    </select>
                </div>
            ...
            <div class="border-slate-850 space-y-4 border-t pt-2">
                ...
                <div>
                    ...
                    >
                        <option value="Singles">1 vs 1 Singles Action</option>
                ...
                <div v-if="bookingMatchType !== 'Segment'" class="space-y-3">
                    ...
                    <!-- Championship line -->
                    <div
                        class="space-y-2 rounded-xl border border-slate-800 bg-slate-950/80 p-3"
                    >
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        ...
                        <div v-if="bookingOutcome === 'Decisive'">
                            ...
                            >
                                <option value="1">
                                    {{ getCompetitorNameBySlot('1') }}
                                </option>
                                <option value="2">
                                    {{ getCompetitorNameBySlot('2') }}
                                </option>
                                <option
                                    v-if="
                                        [
        ...
        <div class="space-y-4 lg:col-span-2">
            ...
            >
                ...
                >
                    Configure parameter options and stage matches above to
                    compile the broadcast card.
                </div>
                <div v-else class="space-y-3">
                    <div
                        ...
                        :class="{
                            'bg-amber-450/5 border-amber-400/50':
                    ...
                    <div class="border-t border-slate-800/60 pt-4">
                        ...
                        >
                            <Sparkles class="h-4 w-4" /> Commit Broadcast Card
                            to Logs
                        </button>
                    </div>
                </div>
            ...
            >
                <div class="relative z-10 flex flex-col items-center space-y-6">
                    ...
                    >
                        <span
                            class="flex items-center gap-1.5 text-xs font-black tracking-widest text-amber-400 uppercase"
                        >
                            <Trophy class="h-4 w-4" /> CHAMPIONSHIP MATCH
                        </span>
                        <span
                            class="text-sm font-black tracking-wide text-white uppercase italic drop-shadow"
                        >
                    ...
                    <div class="flex flex-col items-center text-center">
                        <span
                            class="text-xs font-extrabold tracking-widest text-purple-400 uppercase"
                        >
                            {{
                                ...
                                <div
                                    ...
                                    :class="{
                                        'opacity-40 grayscale':
                                            activeMatchPreview.outcome ===
                                                'Decisive' &&
                                            activeMatchPreview.winnerSlot !==
                                                '1',
                                    }"
                                >
                                    <div class="relative">
                                        ...
                                        <div
                                            v-if="
                                                ...
                                                activeMatchPreview.winnerSlot ===
                                                    '1'
                                            "
                                            class="absolute -top-2 -right-2 rounded-full bg-gradient-to-r from-amber-400 to-yellow-300 px-1.5 py-0.5 text-[8px] font-black tracking-wider text-slate-950 uppercase shadow"
                                        >
                                            🏆 WINNER
                                        </div>
                                    </div>
                                    <span
                                        class="max-w-[100px] truncate text-xs font-bold tracking-wide text-white uppercase"
                                    >
                    ...
                    <!-- Outcome Summary Bar -->
                    <div
                        class="flex w-full flex-col items-center space-y-2 border-t border-slate-800 pt-4 text-center"
                    >
                        <div class="text-xs font-bold">
                            <span
                                v-if="activeMatchPreview.outcome === 'Decisive'"
                            ...
                            <span
                                v-else-if="
                                    activeMatchPreview.outcome === 'Draw'
                                "
